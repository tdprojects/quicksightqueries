...
SUM(CASE 
        WHEN m.date_entered BETWEEN fmc.operations_start_date_c AND DATE_ADD(fmc.operations_start_date_c, INTERVAL 30 DAY)
            THEN 1
            ELSE 0
        END) AS consultations_30_days,
    SUM(CASE 
        WHEN m.date_entered BETWEEN fmc.operations_start_date_c AND DATE_ADD(fmc.operations_start_date_c, INTERVAL 60 DAY)
            THEN 1
            ELSE 0
        END) AS consultations_60_days,
    SUM(CASE 
        WHEN m.date_entered BETWEEN fmc.operations_start_date_c AND DATE_ADD(fmc.operations_start_date_c, INTERVAL 90 DAY)
            THEN 1
            ELSE 0
        END) AS consultations_90_days,
    SUM(CASE 
        WHEN m.date_entered BETWEEN fmc.operations_start_date_c AND DATE_ADD(fmc.operations_start_date_c, INTERVAL 180 DAY)
            THEN 1
            ELSE 0
        END) AS consultations_180_days,
    SUM(CASE 
        WHEN m.date_entered BETWEEN fmc.operations_start_date_c AND DATE_ADD(fmc.operations_start_date_c, INTERVAL 365 DAY)
            THEN 1
            ELSE 0
        END) AS consultations_365_days,
		CASE
